...
pub(crate) fn policy_from_headers(
    ...
    let age = header(headers, "age")
        .and_then(|value| value.parse().ok())
        .map(Duration::from_secs);
    compute_policy_from_directives(&directives, expires, age, default_lifetime)
}
...
fn compute_policy_from_directives(
    ...
    if let Some(expires) = expires {
        return CachePolicy::CacheFor {
            // `Expires` is absolute, so comparison with wall time already
            // accounts for time the response spent in an intermediary cache.
            lifetime: expires_lifetime(expires),
            must_revalidate: directives.must_revalidate,
        };
    }
...
fn expires_lifetime(value: &str) -> Duration {
